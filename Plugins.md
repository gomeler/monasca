...
| Metric Name | Dimensions | Semantics |
| ----------- | ---------- | --------- |
| solidfire.active_block_space        | service=solidfire, cluster | Amount of space consumed by the block services, including cruft |
| solidfire.active_cluster_faults     | service=solidfire, cluster | Amount of active cluster faults, such as failed drives |
| solidfire.active_meta_space         | service=solidfire, cluster | Amount of space consumed by the metadata services |
| solidfire.active_snapshot_space     | service=solidfire, cluster | Amount of space consumed by the metadata services for snapshots |
| solidfire.average_iops              | service=solidfire, cluster | Average IOPs since midnight UTC |
| solidfire.clusterUtilization        | service=solidfire, cluster | Overall cluster IOP utilization |
| solidfire.current_iops              | service=solidfire, cluster | Average IOPs over the last 5 seconds |
| solidfire.iscsi_sessions            | service=solidfire, cluster | Amount of active iSCSI sessions connected to the cluster |
| solidfire.max_block_space           | service=solidfire, cluster | Maximum amount of bytes allocated to the block services |
| solidfire.max_iops                  | service=solidfire, cluster | Theoretical maximum amount of IOPs |
| solidfire.max_meta_space            | service=solidfire, cluster | Maximum amount of bytes allocated to the metadata services |
| solidfire.max_overprovisioned_space | service=solidfire, cluster | Max provisionable space * 5, artificial safety limit |
| solidfire.max_provisioned_space     | service=solidfire, cluster | Max provisionable space if 100% metadata space used |
| solidfire.non_zero_blocks           | service=solidfire, cluster | Number of 4KiB blocks with data after the last garbage collection |
| solidfire.peak_iops                 | service=solidfire, cluster | Peak IOPS since midnight UTC |
| solidfire.provisioned_space         | service=solidfire, cluster | Total number of provisioned bytes |
| solidfire.unique_blocks             | service=solidfire, cluster | Number of blocks(not always 4KiB) stored on block drives |
| solidfire.unique_blocks_used_space  | service=solidfire, cluster | Amount of space the unique blocks take on the block drives |
